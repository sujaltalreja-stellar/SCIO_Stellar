...
export default function AppLayout({ children, activeTab, setActiveTab }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-textBright flex flex-col font-sans selection:bg-emerald-500 selection:text-white">
      {/* HEADER Telemetry Banner */}
      <header className="bg-panel border-b border-borderMuted sticky top-0 z-40 backdrop-blur-md px-6 py-3 flex items-center justify-between shadow-xs">
        <div className="flex items-center space-x-3">
          <div className="bg-emerald-500/10 p-2 rounded-lg border border-emerald-500/20">
            <Activity className="h-6 w-6 text-emerald-600 dark:text-emerald-500 animate-pulse" />
          </div>
          <div>
            <h1 className="text-md font-bold tracking-wider text-textBright flex items-center gap-2">
              STELLAR SCIO <span className="text-emerald-600 dark:text-emerald-500 text-xs font-semibold bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">ENERGY</span>
            </h1>
            <p className="text-[10px] text-textMuted tracking-widest font-mono">OPERATIONS CONTROL CENTER</p>
          </div>
        </div>
        ...
        <div className="hidden lg:flex items-center space-x-8 text-xs font-mono">
          <div className="flex flex-col">
            <span className="text-[9px] text-textMuted uppercase tracking-wider">Total Portfolio Capacity</span>
            <span className="text-sm font-semibold text-textBright">
              {stats?.totalCapacity ? (stats.totalCapacity / 1000).toFixed(2) : "0.00"} GW
            </span>
          </div>
          <div className="flex flex-col border-l border-borderMuted pl-8">
            <span className="text-[9px] text-textMuted uppercase tracking-wider">Live Power Output</span>
          ...
          <div className="flex flex-col border-l border-borderMuted pl-8">
            <span className="text-[9px] text-textMuted uppercase tracking-wider">Capacity Factor Avg</span>
          ...
          <div className="flex flex-col border-l border-borderMuted pl-8">
            <span className="text-[9px] text-textMuted uppercase tracking-wider">Active Alarms</span>
            <span className={`text-sm font-semibold flex items-center gap-1.5 ${
              (stats?.activeAlarmsCount ?? 0) > 0 ? "text-rose-600 dark:text-red-500 font-bold" : "text-textMuted"
            }`}>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2 text-[10px] font-mono bg-panelLight border border-borderMuted px-3 py-1.5 rounded-full">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span className="text-textMuted">SCADA FEED: LIVE</span>
          </div>

          {/* Theme Toggle Button */}
          <button
            onClick={toggleTheme}
            className="p-2 text-textMuted hover:text-textBright hover:bg-panelLight border border-borderMuted rounded-lg transition-all"
          ...
          <button
            ...
            className="relative p-2 text-textMuted hover:text-textBright hover:bg-panelLight border border-borderMuted rounded-lg transition-all"
          >
          ...
          <div className="h-8 w-8 rounded-full border border-borderMuted bg-panelLight flex items-center justify-center text-xs font-mono font-semibold text-emerald-600 dark:text-emerald-500">
            OP
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside
          className={`bg-panel border-r border-borderMuted transition-all duration-300 flex flex-col justify-between z-30 ${
            sidebarCollapsed ? "w-16" : "w-64"
          }`}
        >
          <div className="py-4 px-3 flex-1 flex flex-col space-y-1 overflow-y-auto">
            {menuItems.map((item) => {
                ...
                <button
                  ...
                  className={`w-full flex items-center p-3 rounded-lg text-sm transition-all group ${
                    isActive
                      ...
                      : "text-textMuted hover:text-textBright hover:bg-panelLight border border-transparent"
                  }`}
                >
                  <Icon
                    className={`h-5 w-5 flex-shrink-0 transition-transform group-hover:scale-105 ${
                      isActive ? "text-emerald-600 dark:text-emerald-400" : "text-textMuted"
                    }`}
                  />
                  {!sidebarCollapsed && (
                  ...
                  {!sidebarCollapsed && item.badge !== undefined && item.badge > 0 && (
                    <span className="ml-auto bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 text-[10px] font-bold font-mono px-2 py-0.5 rounded-full">
                      {item.badge}
                    </span>
                  )}
                </button>
          ...
          <div className="p-3 border-t border-borderMuted">
            <button
              onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
              className="w-full flex items-center justify-center p-2 rounded-lg text-textMuted hover:text-textBright hover:bg-panelLight transition-all"
            >
